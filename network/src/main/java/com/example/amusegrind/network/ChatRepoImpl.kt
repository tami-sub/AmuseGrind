...
class ChatRepoImpl @Inject constructor(
    ...
    override fun fetchUsers(): Flow<List<User>> = callbackFlow {
        val listener =
            databaseReference.child(FirePath.getAllUsersPath()).addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    val users = mutableListOf<User>()
                    snapshot.children.forEach { child ->
                        child.child(FirePath.getAllUserBasicDataPath()).getValue(User::class.java)?.let { user ->
    ...
    override fun sendMessage(senderRoom: String, receiverRoom: String, message: Message) {
        mDbRef.child(FirePath.getAllUsersPath()).child(senderRoom).child(FirePath.getMessagesPath()).push()
            .setValue(message)
            .addOnSuccessListener {
                mDbRef.child(FirePath.getAllUsersPath()).child(receiverRoom).child(FirePath.getMessagesPath()).push()
                    .setValue(message)
            }
    }

    override fun receiveMessages(roomId: String): Flow<List<Message>> = callbackFlow {
        val listener = mDbRef.child(FirePath.getAllUsersPath()).child(roomId).child(FirePath.getMessagesPath())
